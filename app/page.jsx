...
function SourcePopover({ src, anchorRect, onClose }) {
  ...
  return (
    <div ref={popRef} style={{ position:"fixed", top:pos.top, left:pos.left, width:260, zIndex:9999, borderRadius:12, background:T.surfaceElevated, border:`1px solid ${T.border}`, boxShadow:`0 12px 40px -8px rgba(0,0,0,0.6), 0 0 0 1px ${T.border}`, padding:"16px", animation:"popIn 0.18s cubic-bezier(0.16,1,0.3,1)" }}>
      ...
      {/* BUG FIX #3: proper <a> tag with target="_blank" and rel="noopener noreferrer" */}
      <a
        href={`https://${src.url}`}
        target="_blank"
        rel="noopener noreferrer"
        onClick={e => e.stopPropagation()}
        style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"8px 12px", borderRadius:8, background:T.primaryMuted, border:`1px solid ${T.primary}20`, textDecoration:"none", cursor:"pointer", transition:"all 0.2s" }}
      >
...
function NavHeader({onBack, showBack, onLogoClick, onAccountClick}) {
  return (
    <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0",borderBottom:`1px solid ${T.border}50`,marginBottom:8}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        {showBack && (
          <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px 6px 8px",borderRadius:8,background:T.surface,border:`1px solid ${T.border}`,cursor:"pointer",color:T.textSecondary,fontFamily:"var(--body)",fontSize:12,fontWeight:500}}>
            {I.back} Back
          </button>
        )}
      ...
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <div
          ...
          title="Account"
          style={{width:32,height:32,borderRadius:8,background:`linear-gradient(135deg,${T.primary}40,${T.accent}30)`,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--body)",fontSize:12,fontWeight:700,color:T.text,cursor:"pointer"}}
        >
          G
        </div>
      </div>
...
function MainTabBar({active,onChange}){
  ...
  return(
    <div style={{display:"flex",gap:2,padding:3,borderRadius:10,background:T.surface,border:`1px solid ${T.border}`}}>
      {tabs.map(tab=>{const a=active===tab.id;return(
        <button key={tab.id} onClick={()=>onChange(tab.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:8,border:"none",background:a?`${T.primary}18`:"transparent",cursor:"pointer",fontFamily:"var(--body)",fontSize:12,fontWeight:a?600:500,color:a?T.text:T.textTertiary,boxShadow:a?`0 0 0 1px ${T.primary}30`:"none",transition:"all 0.2s"}}>
...
function HeroCard({story,onClick}){const[h,setH]=useState(false);return<article onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{position:"relative",borderRadius:16,background:`linear-gradient(135deg,${T.surface},#0F0F1A)`,border:`1px solid ${h?T.primary+"40":T.border}`,overflow:"hidden",cursor:"pointer",transition:"all 0.4s cubic-bezier(0.16,1,0.3,1)",boxShadow:h?"0 20px 60px -15px rgba(0,0,0,0.5)":"0 4px 24px -8px rgba(0,0,0,0.3)"}}><div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,${T.primary},${T.accent})`,opacity:h?1:0.6}}/><div style={{padding:"28px 32px 24px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{display:"flex",alignItems:"center",gap:5,padding:"3px 9px",borderRadius:5,background:`linear-gradient(135deg,${T.primary}18,${T.accent}10)`,border:`1px solid ${T.primary}25`}}><svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M6 1L7.5 4.5L11 5L8.5 7.5L9 11L6 9.5L3 11L3.5 7.5L1 5L4.5 4.5L6 1Z" fill={T.accent}/></svg><span style={{fontFamily:"var(--body)",fontSize:10,fontWeight:700,color:T.accent,letterSpacing:"0.06em",textTransform:"uppercase"}}>Top Story</span></div><TopicTag label={story.topic} color={story.topicColor}/><TrendBadge value={story.trending}/></div><ConfBadge score={story.confidence} size="lg"/></div><h1 style={{fontFamily:"var(--serif)",fontSize:30,fontWeight:700,lineHeight:1.22,color:h?"#FFF":T.text,margin:"0 0 12px",maxWidth:740,transition:"color 0.3s"}}>{story.headline}</h1><p style={{fontFamily:"var(--body)",fontSize:14,lineHeight:1.6,color:T.textSecondary,margin:"0 0 18px",maxWidth:660}}>{story.summary.split(". ").slice(0,2).join(". ")}.</p><div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",paddingTop:14,borderTop:`1px solid ${T.border}`}}><div style={{display:"flex",flexDirection:"column",gap:8}}><SourcePills sourceIds={story.sources} max={6}/><div style={{fontFamily:"var(--mono)",fontSize:10,color:T.textTertiary,display:"flex",gap:8}}><span>{story.timestamp}</span><span style={{opacity:0.3}}>·</span><span>{story.readTime}</span></div></div><LeanBar breakdown={story.leanBreakdown} size="lg"/></div></div></article>;}

function StoryCard({story,onClick}){const[h,setH]=useState(false);return<article onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{borderRadius:14,background:h?`linear-gradient(165deg,${T.surfaceHover},${T.surface})`:T.surface,border:`1px solid ${h?T.border+"cc":T.border}60`,padding:"20px 22px 18px",cursor:"pointer",transition:"all 0.35s",display:"flex",flexDirection:"column",boxShadow:h?"0 8px 32px -8px rgba(0,0,0,0.35)":"none",transform:h?"translateY(-2px)":"none"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:6}}><TopicTag label={story.topic} color={story.topicColor}/><TrendBadge value={story.trending}/></div><ConfBadge score={story.confidence}/></div><h2 style={{fontFamily:"var(--serif)",fontSize:18,fontWeight:600,lineHeight:1.3,color:h?"#FFF":T.text,margin:"0 0 10px",flex:1}}>{story.headline}</h2><p style={{fontFamily:"var(--body)",fontSize:12,lineHeight:1.5,color:T.textSecondary,margin:"0 0 14px",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{story.summary}</p><SourcePills sourceIds={story.sources} max={4}/><div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",paddingTop:12,borderTop:`1px solid ${T.border}50`,marginTop:14}}><div style={{fontFamily:"var(--mono)",fontSize:10,color:T.textTertiary,display:"flex",gap:8}}><span>{story.timestamp}</span><span style={{opacity:0.3}}>·</span><span>{story.readTime}</span></div><LeanBar breakdown={story.leanBreakdown}/></div></article>;}
    ...
    <div className="fade-up">
      ...
      {!topicFilter && (
        <div style={{display:"flex",gap:1,padding:1,borderRadius:10,background:T.border+"40",marginBottom:24}}>
          {[{l:"Sources Active",v:"21",i:"◉"},{l:"Stories Today",v:"147",i:"◈"},{l:"Avg Confidence",v:"87%",i:"◎"},{l:"Updated",v:"2m ago",i:"◷"}].map((s,i,a)=>(
            <div key={i} style={{flex:1,display:"flex",alignItems:"center",gap:10,padding:"9px 14px",background:T.surface,borderRadius:i===0?"9px 0 0 9px":i===a.length-1?"0 9px 9px 0":0}}>
              <span style={{fontSize:13,opacity:0.4}}>{s.i}</span>
              <div><div style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:700,color:T.text,lineHeight:1}}>{s.v}</div><div style={{fontFamily:"var(--body)",fontSize:9,color:T.textTertiary,marginTop:2}}>{s.l}</div></div>
            </div>
          ))}
        </div>
      )}
      {hero && <div style={{marginBottom:32}}><HeroCard story={hero} onClick={()=>onStoryClick(hero)}/></div>}
      <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"baseline",gap:8}}>
          <h2 style={{fontFamily:"var(--serif)",fontSize:20,fontWeight:600,color:T.text,margin:0}}>{topicFilter ? `${topicFilter} Stories` : "Today's Intelligence"}</h2>
...
function DeepDivePage({story, onBack, savedStories, onToggleSave, followedTopics, onToggleFollow, onTopicClick}) {
  ...
  const handleShare = () => {
    const url = `https://undercurrent.news/story/${story.id}`;
    const doFallback = () => {
      try {
  ...
  return (
    <div style={{maxWidth:800,margin:"0 auto"}} className="fade-up">
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:24,fontFamily:"var(--mono)",fontSize:10,color:T.textTertiary}}>
      ...
      <h1 style={{fontFamily:"var(--serif)",fontSize:38,fontWeight:700,lineHeight:1.2,color:T.text,margin:"0 0 16px",letterSpacing:"-0.015em"}}>{story.headline}</h1>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24,fontFamily:"var(--mono)",fontSize:11,color:T.textTertiary}}>
        <span style={{display:"flex",alignItems:"center",gap:4}}>{I.clock} {story.publishedAt}</span>
        <span style={{opacity:0.3}}>·</span>
        <span>{story.readTime} read</span>
      </div>

      {/* Action buttons */}
      <div style={{display:"flex",gap:8,marginBottom:32,paddingBottom:24,borderBottom:`1px solid ${T.border}`,flexWrap:"wrap"}}>
        <button
          onClick={() => onToggleFollow(story.topic)}
          style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:`1px solid ${isFollowed?T.accent:T.primary}`,background:isFollowed?T.accentMuted:T.primaryMuted,cursor:"pointer",fontFamily:"var(--body)",fontSize:12,fontWeight:600,color:isFollowed?T.accent:T.primary}}
        >
        ...
        <button
          ...
          style={{display:"flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:8,border:`1px solid ${T.border}`,background:isSaved?T.surfaceElevated:T.surface,cursor:"pointer",fontFamily:"var(--body)",fontSize:12,fontWeight:500,color:isSaved?T.accent:T.textSecondary}}
        >
          {I.bookmark}
          {isSaved ? "Saved" : "Save"}
        </button>
        <button
          onClick={handleShare}
          style={{display:"flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:8,border:`1px solid ${copied?T.accent+"60":T.border}`,background:copied?T.accentMuted:T.surface,cursor:"pointer",fontFamily:"var(--body)",fontSize:12,fontWeight:500,color:copied?T.accent:T.textSecondary,transition:"all 0.2s"}}
        >
          {copied ? "✓ Copied!" : "↗ Share"}
        </button>
      </div>
      ...
      <div style={{marginBottom:36}}>
        <div style={{fontFamily:"var(--serif)",fontSize:20,fontWeight:600,color:T.text,marginBottom:16}}>Left / Center / Right Lens</div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        <div style={{display:"flex",gap:2,padding:3,borderRadius:10,background:T.surface,border:`1px solid ${T.border}`}}>
          {periodOpts.map(o=>{const a=period===o.id;return(
            <button key={o.id} onClick={()=>setPeriod(o.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 18px",borderRadius:8,border:"none",background:a?`${T.accent}15`:"transparent",cursor:"pointer",fontFamily:"var(--body)",fontSize:12,fontWeight:a?600:500,color:a?T.accent:T.textTertiary,boxShadow:a?`0 0 0 1px ${T.accent}30`:"none"}}>
              {o.label}<span style={{fontFamily:"var(--mono)",fontSize:9,opacity:a?0.7:0.4}}>{o.sub}</span>
            </button>
          );})}
        </div>
        <span style={{fontFamily:"var(--mono)",fontSize:10,color:T.textTertiary}}>Updated 2m ago</span>
      </div>

      <div style={{marginBottom:28}}>
        <h1 style={{fontFamily:"var(--serif)",fontSize:28,fontWeight:700,color:T.text,margin:"0 0 6px"}}>{d.title}</h1>
        ...
        <div style={{display:"flex",gap:1,padding:1,borderRadius:10,background:T.border+"40",marginBottom:20}}>
          {[{l:"Stories",v:d.storyCount},{l:"Topics",v:d.topicCount},{l:"Read Time",v:d.readTime.replace(" read","")},{l:"Sources",v:"21"}].map((s,i,a)=>(
  ...
  return (
    <div className="fade-up" style={{paddingBottom:60}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:28}}>
        ...
        <div>
          <h1 style={{fontFamily:"var(--serif)",fontSize:24,fontWeight:700,color:T.text,margin:0}}>{topic}</h1>
      ...
      {filtered.length === 0 ? (
        <div style={{padding:"60px 32px",textAlign:"center",borderRadius:14,background:T.surface,border:`1px solid ${T.border}`}}>
...
function LandingPage({ onEnterApp }) {
  ...
  const observe = useCallback((id) => (el) => {
    if (!el) return;
    const obs = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setVis(p => ({ ...p, [id]: true })); obs.disconnect(); } }, { threshold: 0.15 });
    obs.observe(el);
  }, []);
    ...
    <div style={{ background: T.bg, color: T.text, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <nav style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <button onClick={onEnterApp} style={{ padding: "8px 20px", borderRadius: 8, background: T.primary, border: "none", cursor: "pointer", fontFamily: "var(--body)", fontSize: 13, fontWeight: 600, color: "#FFF" }}>Open App</button>
        </div>
      </nav>

      <section ref={observe("hero")} style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 28px 60px", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ ...sectionStyle("hero", 0), display: "inline-flex", width: 72, height: 72, borderRadius: 20, background: `linear-gradient(135deg, ${T.primary}, ${T.accent})`, alignItems: "center", justifyContent: "center", marginBottom: 28, boxShadow: `0 8px 40px -8px ${T.primary}40, 0 0 80px -20px ${T.accent}20` }}>
          <svg width="40" height="40" viewBox="0 0 18 18" fill="none"><path d="M9 2C5.5 6 4 8.5 4 11C4 13.76 6.24 16 9 16C11.76 16 14 13.76 14 11C14 8.5 12.5 6 9 2Z" fill="white" fillOpacity="0.95" /><path d="M9 8C7.5 10 7 11 7 12.2C7 13.2 7.9 14 9 14C10.1 14 11 13.2 11 12.2C11 11 10.5 10 9 8Z" fill={T.primary} fillOpacity="0.5" /></svg>
        </div>

        <div style={{ ...sectionStyle("hero", 100) }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 600, color: T.accent, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 16 }}>Know the Current</div>
        </div>

        <h1 style={{ ...sectionStyle("hero", 200), fontFamily: "var(--serif)", fontSize: 56, fontWeight: 700, lineHeight: 1.1, color: T.text, margin: "0 auto 20px", maxWidth: 700, letterSpacing: "-0.025em" }}>
        ...
        <p style={{ ...sectionStyle("hero", 300), fontFamily: "var(--body)", fontSize: 18, lineHeight: 1.65, color: T.textSecondary, maxWidth: 540, margin: "0 auto 36px", fontWeight: 400 }}>
        ...
        <div style={{ ...sectionStyle("hero", 400), display: "flex", alignItems: "center", justifyContent: "center", gap: 12 }}>
          <button onClick={onEnterApp} style={{ padding: "14px 32px", borderRadius: 10, background: `linear-gradient(135deg, ${T.primary}, ${T.primaryStrong})`, border: "none", cursor: "pointer", fontFamily: "var(--body)", fontSize: 15, fontWeight: 600, color: "#FFF", boxShadow: `0 4px 24px -6px ${T.primary}60`, transition: "transform 0.2s" }}>
            Get Started Free
          </button>
          <button style={{ padding: "14px 32px", borderRadius: 10, background: T.surface, border: `1px solid ${T.border}`, cursor: "pointer", fontFamily: "var(--body)", fontSize: 15, fontWeight: 500, color: T.textSecondary }}>
            See How It Works ↓
          </button>
        </div>

        <div style={{ ...sectionStyle("hero", 550), display: "flex", alignItems: "center", justifyContent: "center", gap: 20, marginTop: 40, fontFamily: "var(--mono)", fontSize: 11, color: T.textTertiary }}>
      ...
      <section ref={observe("preview")} style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 28px 60px" }}>
        <div style={{ ...sectionStyle("preview"), textAlign: "center", marginBottom: 28 }}>
        ...
        <div style={{ ...sectionStyle("preview", 150), display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {previewStories.map((s, i) => (
            <div key={s.id} style={{ ...sectionStyle("preview", 200 + i * 100), borderRadius: 14, background: T.surface, border: `1px solid ${T.border}60`, padding: "20px 22px 18px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
                <TopicTag label={s.topic} color={s.topicColor} />
                <ConfBadge score={s.confidence} />
              </div>
              <h3 style={{ fontFamily: "var(--serif)", fontSize: 16, fontWeight: 600, lineHeight: 1.3, color: T.text, margin: "0 0 10px", flex: 1 }}>{s.headline}</h3>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 12, borderTop: `1px solid ${T.border}50`, marginTop: 10 }}>
      ...
      <section ref={observe("features")} style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 28px 80px" }}>
        <div style={{ ...sectionStyle("features"), textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ fontFamily: "var(--serif)", fontSize: 36, fontWeight: 700, color: T.text, margin: "0 0 12px", letterSpacing: "-0.015em" }}>Intelligence, not information</h2>
          <p style={{ fontFamily: "var(--body)", fontSize: 16, color: T.textSecondary, maxWidth: 500, margin: "0 auto" }}>Three systems working together to cut through noise and give you understanding.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[
            { title: "AI Context Engine", desc: "Every story gets a causal chain explaining how we got here and what happens next. Claude analyzes 21 sources to build context that scales with topic complexity — simple summaries for simple stories, deep analysis for complex ones.", icon: <svg width="28" height="28" viewBox="0 0 28 28" fill="none"><rect x="3" y="3" width="22" height="22" rx="5" stroke={T.primary} strokeWidth="1.5"/><path d="M9 9H19M9 14H19M9 19H14" stroke={T.primary} strokeWidth="1.3" strokeLinecap="round"/><circle cx="21" cy="19" r="3" fill={T.primary} fillOpacity="0.2" stroke={T.primary} strokeWidth="1.2"/></svg>, color: T.primary, label: "Core Feature" },
            { title: "Confidence Scoring", desc: "Not all reporting is equal. Every story shows how many sources agree on core facts, which claims are independently verified, and which are analysis or speculation. Transparency you can trust.", icon: <svg width="28" height="28" viewBox="0 0 28 28" fill="none"><circle cx="14" cy="14" r="10" stroke={T.accent} strokeWidth="1.5"/><circle cx="14" cy="14" r="10" stroke={T.accent} strokeWidth="1.5" strokeDasharray="47 63" transform="rotate(-90 14 14)" strokeLinecap="round"/><path d="M10 14L13 17L18 11" stroke={T.accent} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>, color: T.accent, label: "Trust Layer" },
            { title: "Left / Center / Right Lens", desc: "See how the same story looks through different editorial lenses. Not just bias labels — actual summaries of what each perspective argues, with key quotes and sentiment analysis.", icon: <svg width="28" height="28" viewBox="0 0 28 28" fill="none"><rect x="3" y="10" width="6" height="10" rx="2" fill={T.left} fillOpacity="0.2" stroke={T.left} strokeWidth="1.2"/><rect x="11" y="7" width="6" height="13" rx="2" fill={T.center} fillOpacity="0.1" stroke={T.center} strokeWidth="1.2" opacity="0.5"/><rect x="19" y="10" width="6" height="10" rx="2" fill={T.right} fillOpacity="0.2" stroke={T.right} strokeWidth="1.2"/></svg>, color: T.textSecondary, label: "Perspective" },
          ].map((f, i) => (
            <div key={i} style={{ ...sectionStyle("features", 100 + i * 120), padding: "32px 28px", borderRadius: 16, background: T.surface, border: `1px solid ${T.border}`, position: "relative", overflow: "hidden" }}>
              ...
              <span style={{ fontFamily: "var(--mono)", fontSize: 10, fontWeight: 600, color: f.color, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 10, display: "block" }}>{f.label}</span>
              <h3 style={{ fontFamily: "var(--serif)", fontSize: 22, fontWeight: 600, color: T.text, margin: "0 0 12px" }}>{f.title}</h3>
              <p style={{ fontFamily: "var(--body)", fontSize: 14, lineHeight: 1.65, color: T.textSecondary, margin: 0 }}>{f.desc}</p>
            </div>
          ))}
        </div>
      </section>

      <section ref={observe("how")} style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 28px 80px" }}>
        <div style={{ ...sectionStyle("how"), textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ fontFamily: "var(--serif)", fontSize: 36, fontWeight: 700, color: T.text, margin: "0 0 12px" }}>From noise to understanding in seconds</h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
          {[
            ...
            <div key={i} style={{ ...sectionStyle("how", 100 + i * 150), textAlign: "center", padding: "32px 24px" }}>
              <div style={{ display: "inline-flex", width: 56, height: 56, borderRadius: 16, background: i === 1 ? `linear-gradient(135deg, ${T.primary}15, ${T.accent}10)` : T.surface, border: `1px solid ${i === 1 ? T.primary + "30" : T.border}`, alignItems: "center", justifyContent: "center", marginBottom: 20, fontSize: 22, color: i === 1 ? T.primary : T.textTertiary }}>{s.icon}</div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, fontWeight: 700, color: T.accent, letterSpacing: "0.06em", marginBottom: 10 }}>Step {s.step}</div>
              <h3 style={{ fontFamily: "var(--serif)", fontSize: 20, fontWeight: 600, color: T.text, margin: "0 0 10px" }}>{s.title}</h3>
              <p style={{ fontFamily: "var(--body)", fontSize: 14, lineHeight: 1.6, color: T.textSecondary, margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>

      <section ref={observe("pricing")} style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 28px 80px" }}>
        <div style={{ ...sectionStyle("pricing"), textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ fontFamily: "var(--serif)", fontSize: 36, fontWeight: 700, color: T.text, margin: "0 0 12px" }}>{"Start free. Go deep when you're ready."}</h2>
          <p style={{ fontFamily: "var(--body)", fontSize: 16, color: T.textSecondary }}>No credit card required. No ads, ever.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 760, margin: "0 auto" }}>
          <div style={{ ...sectionStyle("pricing", 100), padding: "32px 28px", borderRadius: 16, background: T.surface, border: `1px solid ${T.border}` }}>
            <div style={{ fontFamily: "var(--mono)", fontSize: 11, fontWeight: 600, color: T.textTertiary, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 8 }}>Free</div>
            <div style={{ fontFamily: "var(--serif)", fontSize: 36, fontWeight: 700, color: T.text, marginBottom: 4 }}>$0</div>
            <div style={{ fontFamily: "var(--body)", fontSize: 13, color: T.textTertiary, marginBottom: 24 }}>forever</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
          ...
          <div style={{ ...sectionStyle("pricing", 200), padding: "32px 28px", borderRadius: 16, background: `linear-gradient(165deg, ${T.surface} 0%, #0F1020 100%)`, border: `1px solid ${T.primary}30`, position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, ${T.primary}, ${T.accent})` }} />
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, fontWeight: 600, color: T.primary, letterSpacing: "0.06em", textTransform: "uppercase" }}>Premium</div>
              <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: T.accent, background: T.accentMuted, padding: "2px 8px", borderRadius: 4, fontWeight: 600 }}>Popular</span>
      ...
      <footer ref={observe("footer")} style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 28px 48px", borderTop: `1px solid ${T.border}40` }}>
        <div style={{ ...sectionStyle("footer"), display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            <LogoMark size={22} />
    ...
    <>
      ...
      {mode === "app" && (
        <div style={{ minHeight: "100vh", background: T.bg, color: T.text }}>
          <div style={{ position: "fixed", top: -200, left: "50%", transform: "translateX(-50%)", width: 900, height: 500, borderRadius: "50%", background: `radial-gradient(ellipse, ${T.primary}06 0%, transparent 70%)`, pointerEvents: "none" }} />
          <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 28px", position: "relative", zIndex: 1 }}>
            ...
            <div style={{ borderTop: `1px solid ${T.border}40`, padding: "20px 0 36px", display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
              <span style={{ fontFamily: "var(--body)", fontSize: 10, color: T.textTertiary }}>Undercurrent v1.1</span>
              <div style={{ display: "flex", gap: 14, fontFamily: "var(--mono)", fontSize: 9, color: T.textTertiary }}>
